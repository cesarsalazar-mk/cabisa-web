...
function DocumentTotalCell({
  record,
  totalField = 'total',
  documentNumberField = 'document_number',
  showBillManagementLink = true,
  showCurrencyPrefix = false,
}) {
  const adjustments = record.adjustments || []
  const hasAdjustments = adjustments.length > 0
  const originalTotal = Number(record[totalField] ?? record.total ?? record.total_amount ?? 0)
  const adjustedTotal = Number(record.adjusted_total ?? originalTotal)
  const netAdjustment = Number(record.net_adjustment || 0)
  const documentNumber = record[documentNumberField]

  const goToBillManagement = () => {
    if (!documentNumber) return

    window.open(
      buildBillManagementUrl(documentNumber),
      '_blank',
      'noopener,noreferrer'
    )
  }
  ...
  const renderAmount = value =>
    showCurrencyPrefix ? formatCurrency(value) : formatAmount(value)
      ...
      >
        ...
        <div
          style={{
            display: 'flex',
            justifyContent: 'space-between',
            marginBottom: showBillManagementLink && documentNumber ? 12 : 0,
          }}
        >
        ...
        {showBillManagementLink && documentNumber && (
  ...
  const cellContent = (
    ...
    >
      ...
      {hasAdjustments && (
        ...
        >
          Original: {renderAmount(originalTotal)}
        </span>
      )}
      {renderBadge()}
    </div>
...
export default DocumentTotalCell
